...
set -e
##############################################################################
#  Pipeline 3 — MGS Step 3: Bracken genus estimation on extracted 16S reads
#                           (Patrick SRA Data)
#
#  Runs Bracken on Kraken2 reports from SortMeRNA-extracted 16S reads,
#  then combines all sample-level outputs into a single genus table.
#
#  Prerequisites:
#    - Kraken2 step completed (submit_kraken2_p3_patrick.sh)
...
BR_DIR="${SCRIPT_DIR}/results/patrick/pipeline3/MGS/bracken"
OUT_DIR="${SCRIPT_DIR}/results/patrick/pipeline3/MGS"
...
echo "=== Pipeline 3 — MGS: Bracken genus-level estimation [extracted 16S] (Patrick) ==="
echo ""
...
echo ""
echo "Compare with 16S side results in: results/patrick/pipeline3/16S/"
